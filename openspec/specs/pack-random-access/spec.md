...
Provide index-based random access to packfile objects — resolving an OID to a pack offset via the `.idx` and materializing a single object (with only its delta base chain) — so that object reads do not require parsing an entire packfile.
