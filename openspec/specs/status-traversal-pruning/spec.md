# Status Traversal Pruning

## Purpose

Bound the cost of the `status` working-tree walk on large repositories by pruning ignored directory subtrees, traversing the tree at most once, and keeping per-file filesystem work bounded — without changing which paths are reported.
